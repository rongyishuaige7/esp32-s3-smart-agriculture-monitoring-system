//
// ACD10 I2C CO2 sensor - vendored from https://github.com/RobTillaart/ACD10
//
...
uint8_t ACD10::getAddress() { return _address; }

bool ACD10::preHeatDone() { return preHeatMillisLeft() == 0; }
...
{
  if (_requestStart == 0) return false;
...
int ACD10::readSensor()
{
  ...
  if (buf[2] != _crc8(&buf[0], 2)) return ACD10_CRC_ERROR;
  if (buf[5] != _crc8(&buf[3], 2)) return ACD10_CRC_ERROR;
  if (buf[8] != _crc8(&buf[6], 2)) return ACD10_CRC_ERROR;
...
uint32_t ACD10::getCO2Concentration() { return _concentration; }
uint16_t ACD10::getTemperature() { return _temperature; }
uint32_t ACD10::lastRead() { return _lastRead; }

void ACD10::setRequestTime(uint8_t milliseconds) { _requestTime = milliseconds; }
uint8_t ACD10::getRequestTime() { return _requestTime; }

bool ACD10::setCalibrationMode(uint8_t mode)
{
  if (mode > ACD10_CALIBRATE_AUTO) return false;
  uint8_t buf[5] = {0x53, 0x06, 0x00, 0x00, 0x00};
  buf[3] = mode;
  buf[4] = _crc8(&buf[2], 2);
  return _command(buf, 5) == 0;
}
...
uint16_t ACD10::readManualCalibration()
{
  ...
  _command(buf, 2);
  _request(buf, 3);
  return (uint16_t)buf[0] * 256 + buf[1];
}
...
int ACD10::_request(uint8_t *arr, uint8_t size)
{
  ...
  if (bytes == 0) { _error = -1; return _error; }
  if (bytes < size) { _error = -2; return _error; }
  for (uint8_t i = 0; i < size; i++)
    arr[i] = _wire->read();
  _error = 0;
  return _error;
